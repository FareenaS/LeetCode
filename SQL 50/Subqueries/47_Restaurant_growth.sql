...
--Using CTE's
--For handling duplicates when we have more than one entry for a single date
...
SUM(daily_amount) OVER (ORDER BY visited_on 
                    ROWS BETWEEN CURRENT ROW AND 6 FOLLOWING) AS rolling_sum,                  
ROUND((SUM(daily_amount) OVER (ORDER BY visited_on 
                    ROWS BETWEEN CURRENT ROW AND 6 FOLLOWING)/7.0),2) AS rolling_average,
LEAD(visited_on,6) OVER(ORDER BY visited_on) AS end_date          
FROM Daily_totals
WHERE visited_on BETWEEN visited_on AND DATEADD(day,6,visited_on)
)
SELECT
